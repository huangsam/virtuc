    ...
    fn eval_binary(&self, left: Value, op: BinOp, right: Value) -> Value {
        match (left, right) {
            ...
            (Value::Int(l), Value::Float(r)) => {
                self.eval_binary(Value::Float(l as f64), op, Value::Float(r))
            }
            (Value::Float(l), Value::Int(r)) => {
                self.eval_binary(Value::Float(l), op, Value::Float(r as f64))
            }
...
impl Compiler {
    fn compile(program: &Program) -> (Vec<Opcode>, HashMap<String, usize>) {
        ...
        for function in &program.functions {
            compiler
                .functions
                .insert(function.name.clone(), compiler.code.len());
